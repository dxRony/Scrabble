...
struct Nodo
{
    ...
    Nodo(T nuevoDato) : dato(nuevoDato), siguiente(nullptr) {} // constructor con el primero dato y nullptr (siguiente nodo)
};
...
void ListaEnlazada<T>::eliminar(T valor)
{
    ...
    while (actual) // mientras haya un nodo actual
    {
        if (actual->dato == valor) // si el nodo actual es el nodo a eliminar
        {
            ...
            {
                previo->siguiente = actual->siguiente; // el siguiente del previo se convierte en el siguiente del actual
            }
